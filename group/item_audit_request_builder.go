...
// ItemAuditRequestBuilderPostQueryParameters creates an audit for a group and returns the newly created memberships. Fetch a template for an audit via the 'new' resource.
...
// NextGroup the nextGroup property
// returns a *ItemAuditNextGroupRequestBuilder when successful
func (m *ItemAuditRequestBuilder) NextGroup()(*ItemAuditNextGroupRequestBuilder) {
    return NewItemAuditNextGroupRequestBuilderInternal(m.BaseRequestBuilder.PathParameters, m.BaseRequestBuilder.RequestAdapter)
}
// Post creates an audit for a group and returns the newly created memberships. Fetch a template for an audit via the 'new' resource.
// returns a AuditGroupAuditLinkableWrapperable when successful
// returns a ErrorReport error when the service returns a 4XX or 5XX status code
